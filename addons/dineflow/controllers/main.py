...
class DineFlowAPI(http.Controller):
    ...
    def get_revenue(self):
        ...
        order      = self._get_sort({'date', 'revenue', 'order_count'}, 'date', order_param='sort_order')
        page, size = self._get_page()
        ...
        if with_summary == 'true' or summary_only == 'true':  # MỚI: summary_only cũng trigger build summary
            ...
            revenues = [r.revenue for r in all_records]
            if revenues:
                max_r  = max(all_records, key=lambda r: r.revenue)
                min_r  = min(all_records, key=lambda r: r.revenue)
                wd_rev = [r.revenue for r in all_records
                          if datetime.date.fromisoformat(str(r.date)).weekday() < 5]
                we_rev = [r.revenue for r in all_records
                          if datetime.date.fromisoformat(str(r.date)).weekday() >= 5]
                mid    = len(revenues) // 2
                ...
                summary = {
                    ...
                    'max_revenue_date':   str(max_r.date),
                    'max_revenue':        max_r.revenue,
                    'min_revenue_date':   str(min_r.date),
                    'min_revenue':        min_r.revenue,
